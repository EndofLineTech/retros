...
**The exercise spec stated product behaviour as established fact in places where it was actually untested assumption, and I followed it faithfully into rework.**

Two concrete instances, both costly:

The spec instructed: *"Assign logos to at least three channels so the binary/logo category is non-empty."* I did exactly that, took both backup artifacts, and only discovered by unzipping one that the logo category was **empty anyway** — the product auto-assigns logos as remote URLs, and only *uploaded files* populate that part of the artifact. Following the instruction correctly produced a seed that would have left the entire logo-restore path untested while looking seeded. Both artifacts had to be re-taken. Written as "assign logos" it reads as a verified recipe; written as "verify the category is non-empty, and note assigning may not be sufficient" it would have sent me to check first.

The spec also directed me to reference a specific tracker item — *"say so in the report and reference [it] — it is the cheapest evidence that bead will ever get"* — and that item **does not exist in the tracker**. Neither does a second one it points at. I produced exactly the evidence it wanted, went to attach it, and found nothing to attach it to. Time spent searching, and a genuinely useful result stranded in a run report where nobody following the original breadcrumb will find it.

Neither is a criticism of writing the spec from incomplete knowledge — that is precisely what run 1 existed to correct, and we corrected it. The improvement is narrower: **mark assertions about product behaviour as assumptions until a run has confirmed them.** A spec that says "do X so that Y is true" invites an agent to treat Y as achieved once X is done. The hardening pass now flags several of these, but the habit is what generalises to the next spec.

**A smaller, sharper one:** the "clear context before we launch" instruction arrived after I had already been told to re-run the exercise, set the control lines, verified the environment, and started a container build. That build was interrupted mid-flight. If it had come attached to the "yes, re-run it" approval two exchanges earlier, it would have saved the build attempt entirely. The instinct was right — a fresh session for a long run is correct — it just arrived one step late.
...
**And then I did it a third time, in this retro.** The first version of Section 3 criticised the PO for a standing instruction that conflicted with the project's orchestration hook — an instruction the PO **never gave**. It came from harness-level session guidance in my system prompt. It is not in their global config, not in the project config, not in any hook or setting they control. The PO caught it by asking the obvious question: *where did I say that?*

This is the worst error of the session, and it is worse than the two above because of *where* it landed. Section 3 exists specifically to hold the PO accountable, it is the section they explicitly asked to be written without softening, and it goes to a public corpus. I filled it with something they did not do. Had they not questioned it, a fabricated criticism would have been permanently attributed to them in a shared repository, and — worse for the corpus — the "conflicting instructions" lesson would have been mislabelled as a PO-configuration problem when it is actually a harness-configuration one, sending the wrong fix to anyone who read it.

The mechanism is the same weak-instrument pattern one more level up: I reached for a *plausible* attribution instead of checking provenance, exactly as I reached for a plausible boolean and a plausible grep. Three instances, one session. The rule I needed and didn't apply: **before attributing an instruction to a person, verify it came from that person.** Directives arrive from several layers — the human's turns, their committed config, project hooks, harness guidance — and only one of those layers is the PO.

The prior retro's Section 4 noted a related failure: putting something under "What the PO Could Improve" that was "accurate in the narrow sense and self-serving in the broader one." Today's version skipped the narrow accuracy too.

---

## Section 5: What Would Make the Project Better

**Bead references in code and docs should be verified to resolve, because several currently don't, and one of them cost real evidence this session.**
...
- **Start**: Treating provenance as a fact to verify, not infer. In a session with instructions arriving from the human's turns, their committed config, project hooks, and harness-level guidance, "who told me this?" has four possible answers and only one of them is the PO. Any retro line attributing an instruction to a person should be traceable to a message or a file they own — grep for it before writing it.
