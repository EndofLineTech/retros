...
**The session-level instruction "Do not call the AgentTool unless the user requested it" directly contradicts this project's orchestration hook, and I hit the collision six times without it ever being reconciled.**

At the documentation phase, my edit to a repo file was blocked with: *"this is an onboarded team project — personas implement, the orchestrator dispatches."* The hook doesn't just refuse — it names the required path and it is the only path. So I had two explicit, mutually exclusive instructions and no way to satisfy both. I chose to dispatch, said so, and moved on. Then did it five more times: four engineering dispatches and a version bump.

Each of those was me knowingly acting against a standing instruction because a hook left no alternative. That's a bad position to put an agent in repeatedly, and the failure mode isn't hypothetical: an agent more literal than me stalls at the docs phase and delivers nothing, and an agent less careful stops mentioning that it's overriding the instruction. Either would have been worse than what happened.

The fix is cheap — either exempt hook-mandated dispatch from the prohibition, or drop the blanket prohibition in projects that carry the orchestration hook. What shouldn't persist is both rules being live at once and the agent adjudicating between them in the moment, six times, silently after the first.

**A smaller, sharper one:** the "clear context before we launch" instruction arrived after I had already been told to re-run the exercise, set the control lines, verified the environment, and started a container build. That build was interrupted mid-flight. If the context-clear had come attached to the "yes, re-run it" approval two exchanges earlier, it would have saved the build attempt entirely. The instinct was right — a fresh session for a long run is correct — it just arrived one step late.
